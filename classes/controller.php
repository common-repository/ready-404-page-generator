...
abstract class controllerFhf {
	...
	protected function _createModel($name = '') {
		if(empty($name)) $name = $this->getCode();
		$parentModule = frameFhf::_()->getModule( $this->getCode() );
		$className = '';
		if(import($parentModule->getModDir(). 'models'. DS. $name. '.php')) {
			$className = toeGetClassNameFhf($name. 'Model');
		}
	...
	protected function _createView($name = '') {
		...
		if(import($parentModule->getModDir(). 'views'. DS. $name. '.php')) {
			$className = toeGetClassNameFhf($name. 'View');
		}
	 ...
	 * @example :
	 return array(
			S_METHODS => array(
				'save' => array(FHF_ADMIN),
				'remove' => array(FHF_ADMIN),
				'restore' => FHF_ADMIN,
			),
			S_USERLEVELS => array(
	 ...
	 * Can be used on of sub-array - FHF_METHODS or FHF_USERLEVELS
	 */
