...
class dbFhf {
     ...
     * #__  Wordpress prefix
     * ^__  Store plugin tables prefix (@see FHF_DB_PREF if config.php)
    ...
    static public function prepareQuery($query) {
        ...
        return str_replace(
                array('#__', '^__', '@__'), 
                array($wpdb->prefix, FHF_DB_PREF, $wpdb->prefix. FHF_DB_PREF),
                $query);
    }
    ...
    static public function timeToDate($timestamp = 0) {
        if($timestamp) {
            if(!is_numeric($timestamp))
                $timestamp = dateToTimestampFhf($timestamp);
    ...
    static public function dateToTime($date) {
        ...
        if(strpos($date, FHF_DATE_DL)) return dateToTimestampFhf($date);
        $arr = explode('-', $date);
        return dateToTimestampFhf($arr[2]. FHF_DATE_DL. $arr[1]. FHF_DATE_DL. $arr[0]);
    }
