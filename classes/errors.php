...
class errorsFhf {
    ...
    static public function setSession($error) {
        ...
        if(is_array($error))
            ...
            $sesErrors[] = $error;
        reqFhf::setVar('sesErrors', $sesErrors, 'session');
    }
    static public function init() {
        $fhfErrors = reqFhf::getVar('fhfErrors');
        if(!empty($fhfErrors)) {
            if(!is_array($fhfErrors)) {
                $fhfErrors = array( $fhfErrors );
            }
            $fhfErrors = array_map('htmlspecialchars', array_map('stripslashes', array_map('trim', $fhfErrors)));
            if(!empty($fhfErrors)) {
                self::$current = $fhfErrors;
                add_filter('the_content', array('errorsFhf', 'appendErrorsContent'), 99999);
            }
        }
    }
    ...
    static public function getSession() {
        return reqFhf::getVar('sesErrors', 'session');
    }
    static public function clearSession() {
        reqFhf::clearVar('sesErrors', 'session');
    }
