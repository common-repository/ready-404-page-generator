...
class fileuploaderFhf {
	 ...
	 * @param string $destFhfDir destination for uploaded file, for wp this should be directory in wp-content/uploads/ dir
	 * @param string $filename name of a file that be uploaded
	 */
	public function validate($inputname, $destFhfDir, $destFilename = '') {
		$res = false;
	...
	public function upload() {
		...
		if (isset($upload['type']) && !empty($upload['type'])) {
			...
			//$this->_fileInfo['id'] = frameFhf::_()->getTable('files')->insert($data_to_store);
			$res = true;
	...
	private function createFileName() {
		return utilsFhf::getRandStr(). '-'. utilsFhf::getRandStr(). '-'. utilsFhf::getRandStr(). '-'. utilsFhf::getRandStr();
	}
	/**
