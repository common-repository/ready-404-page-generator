<?php
class modInstallerFhf {
    static private $_current = array();
    /**
     * Install new moduleFhf into plugin
     * @param string $module new moduleFhf data (@see classes/tables/modules.php)
    ...
    static public function install($module, $path) {
        ...
        if(!empty($module) && !empty($path) && is_dir($path)) {
            if(self::isModule($path)) {
                ...
                if($filesMoved) {
                    if(frameFhf::_()->getTable('modules')->exists($module['code'], 'code')) {
                        frameFhf::_()->getTable('modules')->delete(array('code' => $module['code']));
                    }
                    frameFhf::_()->getTable('modules')->insert($module);
                    self::_runModuleInstall($module);
                    self::_installTables($module);
                    return true;
                    /*if(frameFhf::_()->getTable('modules')->insert($module)) {
                        self::_installTables($module);
                        return true;
                    } else {
                        errorsFhf::push(__(array('Install', $module['code'], 'failed ['. mysql_error(). ']')), errorsFhf::MOD_INSTALL);
                    }*/
                } else {
                    errorsFhf::push(__(array('Move files for', $module['code'], 'failed')), errorsFhf::MOD_INSTALL);
                }
            } else
                errorsFhf::push(__(array($module['code'], 'is not plugin module')), errorsFhf::MOD_INSTALL);
        }
        return false;
    }
    static protected function _runModuleInstall($module) {
        ...
        if(!empty($module['ex_plug_dir']))
            $moduleLocationDir = utilsFhf::getPluginDir( $module['ex_plug_dir'] );
        if(is_dir($moduleLocationDir. $module['code'])) {
            importClassFhf($module['code'], $moduleLocationDir. $module['code']. DS. 'mod.php');
            $moduleClass = toeGetClassNameFhf($module['code']);
    ...
    static public function moveFiles($code, $path) {
        if(!is_dir(FHF_MODULES_DIR. $code)) {
            if(mkdir(FHF_MODULES_DIR. $code)) {
                utilsFhf::copyDirectories($path, FHF_MODULES_DIR. $code);
                return true;
            } else 
                errorsFhf::push(__('Can not create module directory. Try to set permission to '. FHF_MODULES_DIR. ' directory 755 or 777'), errorsFhf::MOD_INSTALL);
        } else
            return true;
            //errorsFhf::push(__(array('Directory', $code, 'already exists')), errorsFhf::MOD_INSTALL);
        return false;
    }
    static private function _getPluginLocations() {
        $locations = array();
        $plug = reqFhf::getVar('plugin');
        if(empty($plug)) {
            $plug = reqFhf::getVar('checked');
            $plug = $plug[0];
        }
    ...
    static private function _getModulesFromXml($xmlPath) {
        if($xml = utilsFhf::getXml($xmlPath)) {
            if(isset($xml->modules) && isset($xml->modules->mod)) {
                ...
                if(empty($modules))
                    errorsFhf::push(__('No modules were found in XML file'), errorsFhf::MOD_INSTALL);
                else
                    return $modules;
            } else
                errorsFhf::push(__('Invalid XML file'), errorsFhf::MOD_INSTALL);
        } else
            errorsFhf::push(__('No XML file were found'), errorsFhf::MOD_INSTALL);
        return false;
    }
    /**
    ...
    static public function check($extPlugName = '') {
        ...
        if($modules = self::_getModulesFromXml($locations['xmlPath'])) {
            ...
            foreach($modules as $m) {
                $modDataArr = utilsFhf::xmlNodeAttrsToArr($m);
                if(!empty($modDataArr)) {
                    if(frameFhf::_()->moduleExists($modDataArr['code'])) { //If module Exists - just activate it
                        self::activate($modDataArr);
                    } else {                                           //  if not - install it
                        if(!self::install($modDataArr, $locations['plugDir'])) {
                            errorsFhf::push(__(array('Install', $modDataArr['code'], 'failed')), errorsFhf::MOD_INSTALL);
                        }
                    }
            ...
            errorsFhf::push(__('Error Activate module'), errorsFhf::MOD_INSTALL);
        if(errorsFhf::haveErrors(errorsFhf::MOD_INSTALL)) {
            self::displayErrors();
            return false;
        }
        return true;
	...
	static private function _getCheckRegPlugs() {
		return get_option(FHF_CODE. 'check_reg_plugs', array());
	}
	static private function _updateCheckRegPlugs($newValue) {
		update_option(FHF_CODE. 'check_reg_plugs', $newValue);
	}
	
	static private function _checkActivatedPlugs() {
		$lastTime = get_option(FHF_CODE. 'checked_reg_plugs_time', 0);
		if(!$lastTime || (time() - $lastTime) > (7 * 24 * 3600/* * 0.000001 /*remove last one*/)) {
			...
			update_option(FHF_CODE. 'checked_reg_plugs_time', time());
		}
	}
	static public function activatePlugin($plugName, $activationKey) {
		...
		if($res) {
			...
			$resArray = utilsFhf::jsonDecode($body);
				...
				if($body) {
					...
					if($resArray && is_array($resArray) && isset($resArray['data']) && isset($resArray['data']['plugHasKeys'])) {
						if((int) $resArray['data']['plugHasKeys']) {
							foreach($modules as $m) {
								frameFhf::_()->getModule('options')->getModel('modules')->put(array(
									'code' => $m['code'],
									'active' => 0,
								));
							}
	...
	static private function _addToActivationMessage($plugName, $modules, $locations) {
		...
		if(!isset($currentMessages[ $plugName ])) {
			...
			$newMessage .= ' <a href="#" onclick="toeShowModuleActivationPopupFhf(\''. $plugName. '\'); return false;" class="toePlugActivationNoteLink">'. __('here'). '</a> ';
	...
	static public function getActivationModules() {
		return get_option(FHF_CODE. 'activate_modules', array());
	}
	static public function updateActivationModules($newValues) {
		update_option(FHF_CODE. 'activate_modules', $newValues);
	}
	static public function updateActivationMessages($newValues) {
		update_option(FHF_CODE. 'activate_modules_msg', $newValues);
	}
	static private function _getCheckModules() {
		return get_option(FHF_CODE. 'check_modules', array());
	}
	static private function _updateCheckModules($newValues) {
		update_option(FHF_CODE. 'check_modules', $newValues);
	}
	/**
	...
	static public function checkActivationMessages() {
		...
		if(!empty($currentMessages)) {
			...
			add_action('admin_notices', array('modInstallerFhf', 'showAdminActivationModuleNotices'));
		}
	...
	static private function _checkActivationModules() {
		...
		if(!empty($modules)) {
			foreach($modules as $m) {
				if(frameFhf::_()->getModule($m['code'])) {
					frameFhf::_()->getModule('options')->getModel('modules')->put(array(
						'code' => $m['code'],
						'active' => 0,
					));
	...
	static public function showAdminActivationModuleNotices() {
		$currentMessages = self::getActivationMessages();
		if(!empty($currentMessages)) {
			frameFhf::_()->getModule('messenger')->getController()->getView()->displayAdminModActivationNotices($currentMessages);
		}
	}
	static public function getActivationMessages() {
		return get_option(FHF_CODE. 'activate_modules_msg', array());;
	}
    /**
    ...
    static public function deactivate() {
        ...
        if($modules = self::_getModulesFromXml($locations['xmlPath'])) {
            foreach($modules as $m) {
                $modDataArr = utilsFhf::xmlNodeAttrsToArr($m);
                if(frameFhf::_()->moduleActive($modDataArr['code'])) { //If module is active - then deacivate it
                    if(frameFhf::_()->getModule('options')->getModel('modules')->put(array(
                        'id' => frameFhf::_()->getModule($modDataArr['code'])->getID(),
                        'active' => 0,
                    ))->error) {
                        errorsFhf::push(__('Error Deactivation module'), errorsFhf::MOD_INSTALL);
                    }
                }
            }
        }
    ...
    static public function activate($modDataArr) {
        ...
        if($modules = self::_getModulesFromXml($locations['xmlPath'])) {
            foreach($modules as $m) {
                $modDataArr = utilsFhf::xmlNodeAttrsToArr($m);
                if(!frameFhf::_()->moduleActive($modDataArr['code'])) { //If module is not active - then acivate it
                    if(frameFhf::_()->getModule('options')->getModel('modules')->put(array(
                        'code' => $modDataArr['code'],
                        'active' => 1,
                    ))->error) {
                        errorsFhf::push(__('Error Activating module'), errorsFhf::MOD_INSTALL);
                    } else {
						// For some reason - activation tables didn't worked here
						/*if(isset($modDataArr['code'])) {
							// Retrive ex_plug_dir data from database
							$dbModData = frameFhf::_()->getModule('options')->getModel('modules')->get(array('code' => $modDataArr['code']));
    ...
    static public function displayErrors($exit = true) {
        $errors = errorsFhf::get(errorsFhf::MOD_INSTALL);
        foreach($errors as $e) {
    ...
    static public function uninstall() {
        ...
        if($modules = self::_getModulesFromXml($locations['xmlPath'])) {
            foreach($modules as $m) {
                ...
                frameFhf::_()->getModule('options')->getModel('modules')->delete(array('code' => $modDataArr['code']));
                utilsFhf::deleteDir(FHF_MODULES_DIR. $modDataArr['code']);
            }
        }
    }
    static protected  function _uninstallTables($module) {
        if(is_dir(FHF_MODULES_DIR. $module['code']. DS. 'tables')) {
            $tableFiles = utilsFhf::getFilesList(FHF_MODULES_DIR. $module['code']. DS. 'tables');
            if(!empty($tableNames)) {
                foreach($tableFiles as $file) {
                    $tableName = str_replace('.php', '', $file);
                    if(frameFhf::_()->getTable($tableName))
                        frameFhf::_()->getTable($tableName)->uninstall();
                }
            }
        }
    }
    static public function _installTables($module, $action = 'install') {
		$modDir = empty($module['ex_plug_dir']) ? 
            FHF_MODULES_DIR. $module['code']. DS : 
            utilsFhf::getPluginDir($module['ex_plug_dir']). $module['code']. DS; 
        if(is_dir($modDir. 'tables')) {
            $tableFiles = utilsFhf::getFilesList($modDir. 'tables');
            if(!empty($tableFiles)) {
                frameFhf::_()->extractTables($modDir. 'tables'. DS);
                foreach($tableFiles as $file) {
                    $tableName = str_replace('.php', '', $file);
                    if(frameFhf::_()->getTable($tableName))
                        frameFhf::_()->getTable($tableName)->$action();
                }
            }
        }
    }
