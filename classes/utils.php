...
class utilsFhf {
    ...
    static public function copyDirectories($source, $destination) {
        if(is_dir($source)) {
            ...
            while ( FALSE !== ( $readdirectory = $directory->read() ) ) {
                ...
                if (is_dir($PathDir)) {
                    utilsFhf::copyDirectories( $PathDir, $destination . '/' . $readdirectory );
                    continue;
                }
    ...
    static public function xmlNodeAttrsToArr($node) {
        ...
        foreach($node->attributes() as $a => $b) {
            $arr[$a] = utilsFhf::xmlAttrToStr($node, $a);
        }
        return $arr;
    }
    ...
    static public function getCurrentWPThemeDir() {
        static $themePath;
        if(empty($themePath)) {
            $themePath = get_theme_root(). DS. utilsFhf::getCurrentWPThemeCode(). DS; 
        }
        return $themePath;
    }
    /**
    ...
    static public function getExtModPath($plugName) {
        if(strpos($plugName, 'wp-content/themes') !== false) {  //for modules in theme dir
            return FHF_SITE_URL. str_replace(DS, '/', $plugName). '/';
        } else
    ...
    static public function isThisCommercialEdition() {
        ...
        foreach($commercialModules as $m) {
            if(!frameFhf::_()->getModule($m)) 
                return false;
            if(!is_dir(frameFhf::_()->getModule($m)->getModDir())) 
                return false;
        }
        return true;*/
        foreach(frameFhf::_()->getModules() as $m) {
