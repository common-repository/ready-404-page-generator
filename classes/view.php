<?php
abstract class viewFhf extends baseObjectFhf {
    /* 
     * @deprecated
     */
    protected $_tpl = FHF_DEFAULT;
    /*
	...
	public function getPath($tpl) {
		$path = '';
		$parentModule = frameFhf::_()->getModule( $this->_code );
		// @deprecated - came from other plugins
		//$plTemplate = frameFhf::_()->getModule('options')->get('template');		// Current plugin template
		/*if(empty($plTemplate) || !frameFhf::_()->getModule($plTemplate))
			$plTemplate = '';
		if(file_exists(utilsFhf::getCurrentWPThemeDir(). 'sub'. DS. $code. DS. $tpl. '.php')) {
            $path = utilsFhf::getCurrentWPThemeDir(). 'sub'. DS. $code. DS. $tpl. '.php';
        } elseif($plTemplate && file_exists(frameFhf::_()->getModule($plTemplate)->getModDir(). 'templates'. DS. $code. DS. $tpl. '.php')) {
			$path = frameFhf::_()->getModule($plTemplate)->getModDir(). 'templates'. DS. $code. DS. $tpl. '.php';
	...
	public function getModel() {
		return frameFhf::_()->getModule( $this->_code )->getController()->getModel();
	}
