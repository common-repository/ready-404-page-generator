 ...
 */
if(!function_exists('dateToTimestampFhf')) {
    function dateToTimestampFhf($date) {
        if(empty($a))
            return false;
        $a = explode(FHF_DATE_DL, $date);
...
if(!function_exists('importClassFhf')) {
    function importClassFhf($class, $path = '') {
        if(!class_exists($class)) {
            if(!$path) {
				$classFile = $class;
				if(strpos(strtolower($classFile), FHF_CODE) !== false) {
					$classFile = preg_replace('/'. FHF_CODE. '/i', '', $classFile);
				}
                $path = FHF_CLASSES_DIR. $classFile. '.php';
			}
            return import($path);
            ...
            /*if(class_exists('ReflectionClass')) {   //ReflectionClass supported begining from php5
                ...
                if(strpos($classFile, FHF_DIR) === false) {   //Class is not in our plugin directory
                    ...
                    if(!empty($plugins)) {
                        for($i = 0; $i < count($plugins); $i++) {
                            if(strpos($plugins[$i], FHF_PLUG_NAME) !== false) {   //Let's remove our plugin from list of active plugins
                                unset($plugins[$i]);
                            }
                        }
 ...
 */
if(!function_exists('toeGetClassNameFhf')) {
    function toeGetClassNameFhf($class) {
        $className = '';
		if(class_exists($class. strFirstUp(FHF_CODE))) 
			$className = $class. strFirstUp(FHF_CODE);
		else if(class_exists(FHF_CLASS_PREFIX. $class)) 
            $className = FHF_CLASS_PREFIX. $class;
		else
            $className = $class;
        return $className;
    } 
 ...
 */
if(!function_exists('toeCreateObjFhf')) {
    function toeCreateObjFhf($class, $params) {
        $className = toeGetClassNameFhf($class);
        $obj = NULL;
 ...
 * @return array
 */
if(!function_exists('prepareParamsFhf')) {
	function prepareParamsFhf(&$d=array(), &$options = array()) {
		if (!empty($d['params'])) {
			...
			if (is_array($d['params'])) {
				$params = utilsFhf::jsonEncode($d['params']);
		...
		if(isset($d['code'])) {
			if ($d['code'] == '') {
				$d['code'] = prepareFieldCodeFhf($d['label']).'_'.rand(0, 9999999);
			}
		}
		return $d;
	}
}
if(!function_exists('prepareFieldCodeFhf')) {
	function prepareFieldCodeFhf($string) {   
...
if(!function_exists('toeDateFhf')) {
	function toeDateFhf($format, $timestamp = null) {
		if(is_null($timestamp))
			$timestamp = time();
		return get_date_from_gmt(date('Y-m-d H:i:s', $timestamp), $format);
	}
}
if(!function_exists('toeCurrentTimeFhf')) {
	function toeCurrentTimeFhf($type = 'timestamp', $gmt = 0) {
