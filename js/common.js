...
jQuery.fn.toeDataTableFhf = function(params) {
	
};
function toeDataTableFhf(params) {
	
}
function toeListableFhf(params) {
	...
	this.draw = function(list, count) {
		this.tbody.find('tr').not('.fhfExample').remove();
		var exampleRow = this.tbody.find('.fhfExample');
		for(var i in list) {
			...
			newRow.removeClass('fhfExample').show();
			if(id)
...
toeListableFhf.prototype.clear = function() {
	this.draw([], 0);
}
toeListableFhf.prototype.showEmpty = function() {
	this.clear();
	if(this.params.emptyMsg) {
		var colsCount = this.tbody.find('.fhfExample').find('td').size();
		this.tbody.append( jQuery('<tr><td colspan="'+ colsCount+ '"><i>'+ this.params.emptyMsg+ '</i></td></tr>') );
	}
	
}
toeListableFhf.prototype.getTable = function() {
	return this.table;
}
toeListableFhf.prototype.getTbody = function() {
	return this.tbody;
}
toeListableFhf.prototype.getData = function() {
	return this.list;
}
toeListableFhf.prototype.getRowById = function(id) {
	if(this.list) {
...
toeListableFhf.prototype.setRowById = function(id, data) {
	if(this.list) {
		for(var i in this.list) {
			if(this.list[i].id == id) {
				this.list[i] = data;
...
toeListableFhf.prototype.setColVal = function(rowId, colId, content) {
...
toeListableFhf.prototype.getPage = function() {
	return this.page;
}
toeListableFhf.prototype.removeRowById = function(id) {
	if(this.list && id) {
		this.tbody.find('tr[row_id='+ id+ ']').remove();
	}
	return false;
}
toeListableFhf.prototype.makeRowUneditable = function(id) {
...
function showTableRowActionsFhf(row) {
	if(jQuery(row).find('.row-actions').size()) {
		jQuery(row).find('.row-actions').show();
	}
}
function hideTableRowActionsFhf(row) {
	if(jQuery(row).find('.row-actions').size()) {
		jQuery(row).find('.row-actions').hide();
	}
}

function setCookieFhf(c_name, value, exdays) {
	var exdate=new Date();
