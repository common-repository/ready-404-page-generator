<?php
class modulesModelFhf extends modelFhf {
    public function get($d = array()) {
        if($d['id'] && is_numeric($d['id'])) {
            $fields = frameFhf::_()->getTable('modules')->fillFromDB($d['id'])->getFields();
            $fields['types'] = array();
            $types = frameFhf::_()->getTable('modules_type')->fillFromDB();
            foreach($types as $t) {
                $fields['types'][$t['id']->value] = $t['label']->value;
            }
            return $fields;
        } elseif(!empty($d)) {
            $data = frameFhf::_()->getTable('modules')->get('*', $d);
            return $data;
        } else {
            return frameFhf::_()->getTable('modules')
                ->innerJoin(frameFhf::_()->getTable('modules_type'), 'type_id')
                ->getAll(frameFhf::_()->getTable('modules')->alias().'.*, '. frameFhf::_()->getTable('modules_type')->alias(). '.label as type');
        }
        parent::get($d);
    }
    public function put($d = array()) {
        ...
        if(is_numeric($id) && $id) {
           ...
           /* else
            ...
            if(frameFhf::_()->getTable('modules')->update($d, array('id' => $id))) {
                $res->messages[] = __('Module Updated');
                $mod = frameFhf::_()->getTable('modules')->getById($id);
                $newType = frameFhf::_()->getTable('modules_type')->getById($mod['type_id'], 'label');
                ...
                $res->data = array(
                    ...
                    'code' => $mod['code'], 
                    'type' => $newType,
                    'params' => utilsFhf::jsonEncode($mod['params']),
                    'description' => $mod['description'],
                    'active' => $mod['active'], 
                );
            } else {
                if($tableErrors = frameFhf::_()->getTable('modules')->getErrors()) {
    ...
    public function delete($d = array()) {
        $id = $this->_getIDFromReq($d);
        if(is_numeric($id) && $id) {
            frameFhf::_()->getTable('modules')->delete($d);
        }
    }
    ...
    public function getTabs($modules = array()){
        ...
        if (!empty($modules)) {
            foreach ($modules as $module) {
                if ($module['has_tab'] && frameFhf::_()->getModule($module['code'])) {
                    $moduleTabs = frameFhf::_()->getModule($module['code'])->getTabs();
                    if (!empty($moduleTabs)) {
	...
	public function activatePlugin($d = array()) {
		...
		if(!empty($plugName)) {
			...
			if(!empty($activationKey)) {
				$result = modInstallerFhf::activatePlugin($plugName, $activationKey);
				if($result === true) {
					$allActivationModules = modInstallerFhf::getActivationModules();
					...
					if(!empty($allActivationModules)) {
						foreach($allActivationModules as $i => $m) {
							if($m['plugName'] == $plugName) {
								...
								modInstallerFhf::updateActivationModules($allActivationModules);
								$this->put(array(
									'code' => $m['code'],
									'active' => 1,
								));
					...
					$allActivationMessages = modInstallerFhf::getActivationMessages();
					...
					if(!empty($allActivationMessages) && isset($allActivationMessages[ $plugName ])) {
						...
						modInstallerFhf::updateActivationMessages($allActivationMessages);
					}
					return true;
	...
	public function activateUpdate($d = array()) {
		...
		if(!empty($plugName)) {
			$activationKey = isset($d['activation_key']) ? $d['activation_key'] : '';
			if(!empty($activationKey)) {
				$result = modInstallerFhf::activateUpdate($plugName, $activationKey);
				if($result === true) {
					return true;
				} elseif(is_array($result)) {	// Array with errors
					$this->pushError($result);
