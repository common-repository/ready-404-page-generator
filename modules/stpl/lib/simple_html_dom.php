...
//        $pattern = "/([\w-:\*]*)(?:\#([\w-]+)|\.([\w-]+))?(?:\[@?(!?[\w-]+)(?:([!*^$]?=)[\"']?(.*?)[\"']?)?\])?([\/, ]+)/is";
    ...
    */
	...
	function getAllChildren() {
		...
		if(!empty($children)) {
			foreach($children as $c) {
				$fhfChilds = $c->getAllChildren();
				if(!empty($fhfChilds)) {
					$children = array_merge($children, $fhfChilds);
				}
			}
		}
		return $children;
		...
		if(!empty($styleStr)) {
			...
			foreach($styleParts as $part) {
				...
				if(!empty($styleKeyVal[0])) {
					$values = $styleKeyVal;
					unset($values[0]);
					$styleArr[ $styleKeyVal[0] ] = implode(':', $values);
				}
			}
		}
		return $styleArr;
	...
	function hasClass($class) {
		return ($this->hasAttribute('class') && strstr($this->getAttribute('class'), $class));
	}
